...
\begin{figure} \lstinputlisting{05 design/assets/auth0
    jwt.json} \caption{Auth0 User Data}
  \captiondesc{Specifically, a JWT for a new account created
    with a Github no-reply email; the \code{sub} field is the
...
To restrict who can confirm their attendance at a job, they
will be assigned to each job in the database, along with an
expected date/time for their check-in and check-out.
...
an employee attempted to distribute the exchange data.
...
decision which is not of concern in this project.

% TODO \paragraph{Verification Factors}
